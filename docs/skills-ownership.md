...
The Pi skills under `skills/` are original Deus documents written for this release. The Apache-2.0 license in the repository covers them. The `skills.lock.json` file records each packaged path, its ownership marker, and the SHA-256 digest used by build-time and runtime integrity checks.
