...
import { runProcess, succeeded, type ProcessResult } from './process.js';
...
async function resolveExecutablePath(executable: string, cwd: string): Promise<string | null> {
...
export async function verifyCommitSignature(
  ...
  const fail = (error: string): SignatureVerification => ({
    commit: '',
    fingerprint: null,
    error,
    diagnostics,
  });
  if (trusted.length === 0) return fail('DEUS_DEXTER_TRUSTED_FINGERPRINTS is empty or malformed');

  const exePath = await resolveExecutablePath(executable, cwd);
  if (!exePath) return fail(`cannot resolve executable: ${executable}`);
  ...
  try {
    ...
    if (!fingerprint) return fail('no VALIDSIG status in verify-commit output');
