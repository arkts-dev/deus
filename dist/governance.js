...
export const banned = (text) => BANNED.flatMap(([name, re]) => [...text.matchAll(re)].map((match) => `${name}: ${match[0]}`)).slice(0, 8);
export function duplicates(body) {
    const seen = new Set();
    const found = [];
...
const STOP = new Set('the and for with that this from into when then than are was were must shall may can its it a an of to in on or as is be not never no without cannot issue title task epic'.split(' '));
export const terms = (text, limit = 200) => [
    ...new Set(strip(text)
        .replace(/^#{1,6}.*$/gm, ' ')
        .toLowerCase()
        .replace(/[^a-z0-9 ]/g, ' ')
...
const negative = (sentence) => /\b(not|never|no|without|cannot)\b/i.test(sentence);
export function contradictions(body, limit = 10) {
    ...
    for (let left = 0; left < sentences.length && found.length < limit; left += 1)
        for (let right = left + 1; right < sentences.length && found.length < limit; right += 1) {
            ...
            for (const token of a)
                if (b.has(token))
                    shared += 1;
            if (shared / (a.size + b.size - shared) >= 0.8 &&
