...
export function webConfigFromEnv() {
    return {
        ...
        searchProvider: process.env.DEUS_WEB_PROVIDER,
    };
}
export function webSearchProvider(config) {
    const provider = config.searchProvider ?? (config.exaKey ? 'exa' : config.searxngUrl ? 'searxng' : 'fetch');
    if (!['exa', 'searxng', 'brave', 'tavily', 'fetch'].includes(provider) ||
...
async function boundedResponse(response, maxBytes = 256_000) {
    if (!response.ok)
    ...
    try {
        while (true) {
            ...
            if (count > maxBytes)
                throw new Error('Web source exceeds output budget');
            parts.push(value);
        }
    }
    finally {
...
export async function searchWeb(query, config, signal) {
    if (!query.trim() || query.length > 4000)
        throw new Error('Invalid search question');
    const provider = webSearchProvider(config);
    const timeout = signal
        ? AbortSignal.any([signal, AbortSignal.timeout(20_000)])
        : AbortSignal.timeout(20_000);
    if (provider === 'brave' || provider === 'tavily')
        return searchWithWebAccess(provider, query, provider === 'brave' ? config.braveKey : config.tavilyKey, timeout);
    if (provider === 'exa') {
...
export async function researchWeb(question, config = webConfigFromEnv(), runtime, signal) {
    ...
    try {
        ...
        let requests = 0, turns = 0, stopped = false;
        const maxRequests = 12;
        const consume = () => {
            if (requests >= maxRequests)
                throw new Error('research_request_budget_exhausted');
            requests++;
        };
        ...
        tools.push({
            ...
            description: 'Verified findings require a source receipt and exact inspected quote. Separate inference and gaps.',
            parameters: Type.Object({
                coverage: Type.Union([Type.Literal('complete'), Type.Literal('partial')]),
                findings: Type.Array(Type.Object({
                    statement: Type.String(),
                    provenance: Type.Union(['external_verified', 'inference', 'parametric_unverified', 'gap'].map((x) => Type.Literal(x))),
        ...
        if (!model)
            return { status: 'unavailable', reason: 'No model configured' };
        ...
        try {
            await session.prompt(JSON.stringify({
                ...
                instruction: 'Use public sources only. Submit research_report with receipt-backed findings, separate inferences, and gaps.',
