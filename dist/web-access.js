...
async function invoke(request, credential, signal) {
    ...
    try {
        ...
        return await new Promise((resolve, reject) => {
            const child = spawn(process.execPath, ['--import', 'tsx', fileURLToPath(new URL('./web-access-worker.js', import.meta.url))], {
                env: {
            ...
            child.on('close', (code) => {
                if (code !== 0)
                    reject(new Error('pi_web_access_failed: ' + stderr.slice(0, 300)));
                else {
                    try {
...
export async function searchWithWebAccess(provider, query, key, signal) {
    return invoke({ kind: provider, query }, { name: provider === 'brave' ? 'BRAVE_API_KEY' : 'TAVILY_API_KEY', value: key }, signal);
}
export async function extractPublicPdf(bytes, url, signal) {
    const value = (await invoke({ kind: 'pdf', bytes: bytes.toString('base64'), url }, undefined, signal));
    const body = value.text?.split('---\n\n')[1]?.split('\n---\n\n*[Truncated')[0]?.trim();
    if (!body)
        throw new Error('source_inaccessible: PDF has no extractable text');
