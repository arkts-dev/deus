...
export async function pinDestination(url, resolve = lookup) {
...
export function readableText(raw, type) {
...
export async function fetchPublic(url, signal, consume = () => { }) {
    ...
    for (let redirects = 0; redirects <= 3; redirects++) {
        ...
        if (!text.trim())
            throw new Error('source_inaccessible: empty readable source');
        return {
            url: target.href,
            text,
            sha256: digest(text),
