...
export function classify(issue, objective) {
    if (terminal(issue))
    ...
    const matched = terms(`${issue.title}\n${issue.body}`, 100).filter((term) => wanted.has(term.toLowerCase()));
    if (matched.length)
        return {
            id: issue.id,
    ...
    if (signals.length)
        return { id: issue.id, classification: 'NOISE', evidence: signals };
    return {
        id: issue.id,
        classification: 'PRODUCT',
...
export function preconditions(issue, board) {
    const errors = [];
    if (terminal(issue))
        errors.push(`already ${issue.status}`);
    if (issue.claimed)
        errors.push('active claim');
    if (issue.workdir)
...
export function planBoard(board) {
    ...
    const byId = new Map(board.issues.map((issue) => [issue.id, issue]));
    ...
    const after = board.issues.filter((issue) => !terminal(issue) &&
        ...
        issue.deps.every((dep) => closedAfter.has(dep))).length;
    return {
        objectiveTerms,
        classifications,
        closes,
...
export default function board(pi) {
    ...
    pi.registerTool({
        ...
        description: 'Parse the forge into a typed board: the dependency frontier, claims, and the dispatchable set.',
